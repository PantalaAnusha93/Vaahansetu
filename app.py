...
def create_tables():
    ...
    try:
        ...
        # NEW TABLE: Centralized BLOB Storage
        cursor.execute("""
        ...
        """)
        conn.commit()
        print("Table 'app_files' checked/created successfully.")
...
def send_otp_email(recipient_email, otp_code, username):
    ...
    <body>
        <div class="container">
            ...
            <div class="footer">
                &copy; {datetime.now().year} VaahanSetu. All rights reserved.
            </div>
        </div>
    </body>
    </html>
    ...
    em['Subject'] = "Your One-Time Password (OTP) for VaahanSetu" # Subject should be simple
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                ...
                <p style="color: #6b7280; font-size: 14px;">We received a request to reset your VaahanSetu driver password.</p>
            ...
            <div class="footer">
                &copy; {datetime.now().year} VaahanSetu. Secure System.
            </div>
        </div>
    </body>
    </html>
...
def send_welcome_email(recipient_email, username):
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>Welcome to VaahanSetu!</h1>
            </div>
            <div class="content">
                <p class="greeting">Hello, {username},</p>

                <p style="font-size: 16px; line-height: 1.5;">
                    Congratulations! Your account has been successfully verified. You are now ready to explore **VaahanSetu**, the real-time smart booking platform connecting customers with goods vehicle services.
                </p>
                ...
                <div class="team-section">
                    <h4>Developed by Team THE ORIGINAL GANGSTERS:</h4>
                    <ul>
                        <li>Pantala Anusha</li>
                        <li>Tera Navya Sri</li>
                        <li>Kotaru Chaitanya Lakshmi</li>
                        <li>Yangareddy Charvitha</li>
                    </ul>
                </div>
    ...
    em['From'] = EMAIL_SENDER
    em['To'] = recipient_email
    em['Subject'] = "🥳 Welcome to VaahanSetu! Your Account is Ready."
...
def send_booking_confirmation_email(recipient_email, customer_name, driver_name, pnr, pickup, drop, fare):
    ...
    <body>
        <div class="container">
            ...
            <div class="footer">
                &copy; {datetime.now().year} VaahanSetu VaahanSetu. All rights reserved.
            </div>
        </div>
    </body>
    </html>
    ...
    <body>
        <div class="email-container">
            ...
            <div class="content">
                ...
                <p class="instructions">We received a request to reset the password for your VaahanSetu customer account. Use the code below to verify your identity.</p>
            ...
            <div class="footer">
                &copy; {datetime.now().year} VaahanSetu Logistics. All rights reserved.<br>
...
# --- NEW: BLOB File Management Functions ---
def save_file(file_storage, field_name, identifier):
    if file_storage and file_storage.filename != '' and allowed_file(file_storage.filename):
        ...
        if conn:
            ...
            try:
                cursor.execute(
                    "INSERT INTO app_files (id, filename, mimetype, file_data) VALUES (%s, %s, %s, %s)",
                    (file_id, filename, mimetype, file_data)
                )
                conn.commit()
                return file_id
            except Error as e:
                print(f"Error saving BLOB to DB: {e}")
            finally:
                cursor.close()
                conn.close()
    return None
...
def delete_file(file_id):
    ...
    if file_id and not ('/' in file_id or '\\' in file_id):
        conn = get_db_connection()
        if conn:
            cursor = conn.cursor()
            try:
                cursor.execute("DELETE FROM app_files WHERE id = %s", (file_id,))
                conn.commit()
            except Error as e:
                print(f"Error deleting BLOB from DB: {e}")
            finally:
                cursor.close()
                conn.close()
...
def driver_register():
    if request.method == 'POST':
        ...
        # Save all files using your existing helper function
        ...
        # --- NEW: Save the UPI QR Code Image ---
        upi_qr_path = save_file(files.get('upiQrCode'), 'upi_qr', data['contactNumber'])
        ...
        try:
            ...
            # --- MODIFIED: Added upi_id and upi_qr_code to the SQL Query (30 placeholders) ---
            sql = """
                INSERT INTO drivers (
                    ...
                    acc_holder_name, acc_number, ifsc_code, bank_name, password, upi_id, upi_qr_code
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """
            
            # --- MODIFIED: Appended data['upiId'] and upi_qr_path to the tuple ---
            values = (
                ...
                data['upiId'], upi_qr_path  # <-- NEW UPI FIELDS
            )
...
def qr_code(app_id):
    ...
    try:
        ...
        qr.add_data(f"VaahanSetu Application ID: {app_id}")
        qr.make(fit=True)
...
def get_application_data(app_id):
    ...
    try:
        ...
        if app_data:
            ...
            profile_photo_url = get_public_url(app_data.get('profile_photo_path'))
    ...
    try:
        ...
        for app in raw_applications:
            combined_app = {
                ...
                # Documents & Info - mapped with get_public_url()
                'profile_photo_path': get_public_url(app['profile_photo_path']),
                'vehicle_photo_path': get_public_url(app['vehicle_photo_path_vehicle'] if app['vehicle_photo_path_vehicle'] else app['vehicle_photo_path']),
                'license_number': app['license_number'],
                'license_expiry_date': app['license_expiry_date'],
                'driving_license_path': get_public_url(app['driving_license_path']),
                'aadhaar_path': get_public_url(app['aadhaar_path']),
                'pan_path': get_public_url(app['pan_path']),
                'rc_path': get_public_url(app['rc_path_vehicle'] if app['rc_path_vehicle'] else app['rc_path']),
                'insurance_path': get_public_url(app['insurance_path_vehicle'] if app['insurance_path_vehicle'] else app['insurance_path']),
                'dob': app['dob'],
                'email': app['email'],
                'address': app['address'],
                'city': app['city'],
                ...
                'upi_id': app['upi_id'],
                'upi_qr_code': get_public_url(app['upi_qr_code'])
            }
...
def delete_driver(app_id):
    ...
    try:
        ...
        # First, get the file IDs before deleting the record
        ...
        # If the record was found and deleted, now delete the associated BLOBs from DB
        if driver_files:
            for file_id in driver_files.values():
                if file_id:
                    delete_file(file_id)
...
def admin_delete_vehicle(vehicle_id):
    ...
    try:
        ...
        # If the record was found and deleted, now delete the associated BLOBs from DB
        if vehicle_files:
            for file_id in vehicle_files.values():
                if file_id:
                    delete_file(file_id)
    ...
    try:
        ...
        if driver:
            ...
            for vehicle in vehicles:
                ...
                # --- ADDED THIS LINE: Generate the BLOB URL for the frontend ---
                vehicle['photo_url'] = get_public_url(vehicle.get('vehicle_photo_path'))
...
def edit_vehicle(vehicle_id):
    ...
    try:
        ...
        # Fetch existing vehicle data to get old file IDs for deletion
        ...
        # Process new file uploads and keep old BLOB IDs if not updated
        ...
        if new_vehicle_photo and new_vehicle_photo.filename != '':
            ...
            if existing_vehicle['vehicle_photo_path'] and existing_vehicle['vehicle_photo_path'] != vehicle_photo_path:
                delete_file(existing_vehicle['vehicle_photo_path'])
        
        new_rc = files.get('rc')
        if new_rc and new_rc.filename != '':
            rc_path = save_file(new_rc, 'rc', str(vehicle_id))
            if existing_vehicle['rc_path'] and existing_vehicle['rc_path'] != rc_path:
                delete_file(existing_vehicle['rc_path'])
        ...
        if data.get('hasInsurance') == 'yes':
            ...
            # If the old insurance path exists and is being replaced, delete the old BLOB
            if insurance_path != existing_vehicle['insurance_path'] and existing_vehicle['insurance_path']:
                delete_file(existing_vehicle['insurance_path'])
        else:
            insurance_path = None
            insurance_policy_no = None
            insurance_expiry_date = None
            # If insurance is removed, delete the old insurance BLOB
            if existing_vehicle['insurance_path']:
                delete_file(existing_vehicle['insurance_path'])
...
def driver_delete_vehicle(vehicle_id):
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        # Fetch file IDs before deleting the record
        cursor.execute("SELECT vehicle_photo_path, rc_path, insurance_path FROM vehicles WHERE id = %s", (vehicle_id,))
        vehicle_files = cursor.fetchone()
        ...
        # Delete associated BLOBs if they exist
        if vehicle_files:
            for file_id in vehicle_files.values():
                if file_id:
                    delete_file(file_id)
...
def get_public_url(file_id):
    if not file_id:
        return None
    # Fallback for hardcoded static assets
    if file_id.startswith('static/') or '/' in file_id:
        relative_path = file_id.replace(os.sep, '/')
        if relative_path.startswith('static/'):
            return url_for('static', filename=relative_path[len('static/'):])
        return url_for('static', filename=relative_path)
    # Serve from Database
    return url_for('serve_file', file_id=file_id)


@app.route('/file/<file_id>')
def serve_file(file_id):
    """Retrieves and streams the BLOB from the database to the user."""
    conn = get_db_connection()
    if conn:
        cursor = conn.cursor(dictionary=True)
        try:
            cursor.execute("SELECT filename, mimetype, file_data FROM app_files WHERE id = %s", (file_id,))
            record = cursor.fetchone()
...
def my_profile():
    ...
    try:
        ...
        if driver:
            ...
            driver['upi_qr_url'] = get_public_url(driver.get('upi_qr_code'))
            profile_data = driver
...
def update_document(file_storage, field_name, identifier, old_file_id):
    if file_storage and file_storage.filename != '' and allowed_file(file_storage.filename):
        new_file_id = save_file(file_storage, field_name, identifier)
        if new_file_id and old_file_id and 'default_profile.png' not in old_file_id.lower():
...
def update_profile():
    ...
    try:
        ...
        cursor.execute("SELECT id, profile_photo_path, driving_license_path, aadhaar_path, pan_path, upi_qr_code FROM drivers WHERE email = %s", (driver_email,))
        ...
        new_upi_qr_path = update_document(files.get('upiQrCodeFile'), 'upi_qr', identifier, driver_current.get('upi_qr_code'))
        ...
        sql = """
            UPDATE drivers SET 
                full_name = %s, contact_number = %s, address = %s, city = %s, pincode = %s,
                license_number = %s, license_expiry_date = %s, acc_holder_name = %s,
                acc_number = %s, ifsc_code = %s, bank_name = %s, profile_photo_path = %s,
                driving_license_path = %s, aadhaar_path = %s, pan_path = %s,
                upi_id = %s, upi_qr_code = %s
            WHERE email = %s
        """
        values = (
            data['fullName'], data['contactNumber'], data['address'], data['city'], data['pincode'],
            data['licenseNumber'], license_expiry_date, data['accHolderName'],
            data['accNumber'], data['ifscCode'], data['bankName'], new_profile_photo_path, 
            new_dl_path, new_aadhaar_path, new_pan_path,
            data.get('upiId'), new_upi_qr_path,  # <-- NEW UPI FIELDS ADDED HERE
            driver_email
        )
...
def process_booking():
    ...
    toll_preference = data.get('tollPreference')
    ...
    # --- NEW: APPLY 15% DISCOUNT FOR "PAY AT PLAZA" ---
    final_estimated_price = pricing_result['estimated_price']
    if toll_preference == 'pay_at_plaza':
        final_estimated_price = round(final_estimated_price * 0.85, 2)
    # --------------------------------------------------
        ...
        try:
            ...
            for vehicle in db_vehicles:
                ...
                if ai_price_baseline > 0:
                    ...
                    # Fallback if AI price was 0 or missing
                    driver_specific_price = formatted_vehicle['price_raw']
                    if booking_data.get('tollPreference') == 'pay_at_plaza':
                        driver_specific_price = round(driver_specific_price * 0.85) # Apply 15% discount
...
def confirm_final_booking():
    ...
    # Extract the payment method from the frontend request (Defaults to Cash if missing)
    payment_method = incoming_data.get('payment_method', 'Cash Mode')

    toll_pref = booking_data.get('tollPreference')
    final_fare = f"{base_price} (Price Includes Toll Tax)" if toll_pref == 'prepaid' else f"{base_price} + Extra Tolls"

    customer_name = booking_data.get('fullName', 'VaahanSetu User')
    ...
    try:
        # Added payment_method to the INSERT query
        sql = """
            INSERT INTO trips (
                booked_by, customer_name, customer_email, customer_phone, 
                customer_gender, customer_age, vehicle_type_booked,
                pickup_location, pickup_lat, pickup_lng, 
                drop_location, drop_lat, drop_lng, fare, load_weight, status, payment_method
        ...
        values = (
            ...
            payment_method # Injected here
        )
        cursor.execute(sql, values)
        trip_id = cursor.lastrowid
        conn.commit()

        # Update Socket payload (Optional: You can send payment method to driver too)
        trip_payload = {
            'trip_id': trip_id,
...
def send_trip_completion_email(
    recipient_email,
    customer_name,
    pnr,
    base_fare,
    ...
    total_fare,
    payment_method,
    payment_url  # <--- Changed from trip_id to payment_url
):
    """
    Sends trip completion + payment summary email.
    Shows Razorpay Pay Now button ONLY if payment is pending.
    """

    # ------------------ RAZORPAY PAYMENT SECTION ------------------
    pay_now_section = ""

    # FIX: Ensure all items in the list are strictly lowercase
    if payment_method.lower() in ["online payment", "razorpay", "pending"]:
        pay_now_section = f"""
        <div style="text-align: center; margin-top: 30px;
                    padding: 25px; background: #f0fdf4;
                    border-radius: 14px; border: 2px dashed #22c55e;">
            
            <p style="margin-top: 0; color: #166534;
                      font-weight: 800; font-size: 16px;
                      text-transform: uppercase;">
                Payment Pending
            </p>

            <p style="color: #14532d; font-size: 15px; margin-bottom: 20px;">
                Please complete your payment of
                <strong style="font-size: 18px;">₹ {total_fare:,.2f}</strong>
                to close this trip.
            </p>
            ...
            <p style="margin-top: 15px; font-size: 12px; color: #15803d;">
                You will be redirected to our secure Razorpay checkout page.
            </p>
        </div>
        """
    # -------------------------------------------------------------
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                ...
                <p>Your shipment for Booking ID <strong>{pnr}</strong> has been delivered successfully.</p>
                
                <div class="receipt-card">
                    <h3 style="margin-top:0; border-bottom: 2px solid #0f172a; padding-bottom: 5px;">Payment Summary</h3>
                    <div class="price-row"><span>Base Trip Fare</span><span>₹ {base_fare:,.2f}</span></div>
                    <div class="price-row"><span>Toll & Extra Charges</span><span>₹ {toll_tax:,.2f}</span></div>
                    <div class="total-row"><span>Grand Total</span><span>₹ {total_fare:,.2f}</span></div>
                ...
                <div style="text-align: center; margin-top: 30px;">
                    <p style="font-style: italic; color: #475569;">“Thank you for choosing VaahanSetu.”</p>
                </div>
            </div>
            <div class="footer">
                © {datetime.now().year} VaahanSetu. All rights reserved.
            </div>
        </div>
    </body>
    </html>
    ...
    em['From'] = EMAIL_SENDER
    em['To'] = recipient_email
    em['Subject'] = f"Trip Summary & Payment | {pnr}"
    em.set_content(html_body, subtype='html')
    ...
    try:
        ...
        print(f"Error sending trip completion email: {e}")
        return False
...
def complete_trip(trip_id):
    ...
    try:
        ...
        # SECURE OVERRIDE: Get payment_method directly from the database
        db_payment_method = trip.get('payment_method') or 'Cash Mode'
        ...
        sql = """
            ...
            WHERE id = %s
        """
        cursor.execute(sql, (toll_tax, total_sum, trip_id))
        conn.commit()

        # --- FIX: Generate the Razorpay URL HERE, while request is active ---
        payment_url = url_for('pay_now', trip_id=trip_id, _external=True)

        # 4. Trigger Threaded Email
        def send_async_completion(app_instance, recipient, name, pnr, base, toll, total, method, p_url):
            with app_instance.app_context():
                send_trip_completion_email(recipient, name, pnr, base, toll, total, method, p_url)

        import threading
        email_thread = threading.Thread(
            target=send_async_completion,
            args=(
                app,  # Pass the global 'app' object directly
                trip['customer_email'],
                trip['customer_name'],
                trip['pnr_number'],
                base_fare_num,
                ...
                total_sum,
                db_payment_method,
                payment_url  # Pass the securely generated Razorpay link!
            )
        )
        email_thread.start()
...
def manage_drivers():
    ...
    try:
        ...
        for d in drivers_raw:
            ...
            d['upi_qr_url'] = get_public_url(d.get('upi_qr_code'))
...
def pay_now(trip_id):
    ...
    return render_template("pay_now.html", trip=trip)

@app.route('/pay_upi/<int:trip_id>')
def pay_upi(trip_id):
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    
    # Fetch trip AND the assigned driver's UPI details
    cursor.execute("""
        SELECT t.*, d.upi_id, d.upi_qr_code 
        FROM trips t
        JOIN drivers d ON t.driver_id = d.id
        WHERE t.id = %s
    """, (trip_id,))
    ...
    if not trip:
        flash("Invalid payment link or driver not assigned yet.", "danger")
        return redirect(url_for("home"))
        
    # Convert QR code path to public URL for the template
    if trip.get('upi_qr_code'):
        trip['upi_qr_url'] = get_public_url(trip['upi_qr_code'])
    else:
        trip['upi_qr_url'] = None
        
    return render_template("pay_upi.html", trip=trip)

@app.route('/confirm_upi_payment/<int:trip_id>', methods=['POST'])
def confirm_upi_payment(trip_id):
    """Updates the trip status when the customer marks it as paid via UPI."""
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("""
            UPDATE trips
            SET payment_method = 'Direct UPI',
                status = 'Completed'
            WHERE id = %s
        """, (trip_id,))
        conn.commit()
